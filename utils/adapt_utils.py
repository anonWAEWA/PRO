...
class FWScheduler:
    def __init__(self, optimizer: torch.optim.Optimizer) -> None:
        """Apply this scheduler after every optimizer.step()"""
        self.optimizer = optimizer
        self.initial_lrs = [group["lr"] for group in optimizer.param_groups]
        self.initial_momentums = [group["momentum"] for group in optimizer.param_groups]
        self.n_steps = 1
    ...
    def step(self):
        ...
        for group_idx, group in enumerate(self.optimizer.param_groups):
            group["lr"] = (9 / (self.n_steps + 8)) * self.initial_lrs[group_idx]
            group["momentum"] = (
                4 / (self.n_steps + 7) ** (2 / 3)
            ) * self.initial_momentums[group_idx]
